...
import { Phone, Users, BarChart , ChevronDown , Menu } from 'lucide-react';
          ...
          <div className="flex flex-col-reverse lg:flex-row items-center justify-between gap-12">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4">
                <button  onClick={() => navigate('/packOne')} className="bg-purple-700 hover:bg-purple-800 text-white py-3 px-8 rounded-lg shadow-lg transition-all font-medium">
                  Get Started
                </button>
    ...
    return (
      ...
      >
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div 
              className="md:hidden fixed top-[120px] right-0 bottom-0 w-full sm:w-64 bg-white shadow-xl z-50 overflow-y-auto"
            ...
            >
              <div className="p-4">
                {navItems.map((item) => (
                  <div key={item.name} className="mb-4">
                    {!item.dropdown ? (
                      <Link to={item.path}>
                        <motion.button 
                          className={`${activeTab === item.name ? 'text-purple-600 font-medium' : 'text-gray-700'} w-full text-left py-2 flex justify-between items-center`}
                          onClick={() => {
                            setActiveTab(item.name);
                      ...
                      <>
                        ...
                        {item.dropdown && openDropdown === item.name && (
                          <motion.div 
                            className="ml-4 mt-2 border-l-2 border-purple-100 pl-4"
                ...
                <motion.a 
                  href="tel:+1234567890"
                  className="mt-6 w-full bg-black text-white flex items-center justify-center py-3 rounded-lg font-medium"
                  whileHover={{ scale: 1.02, backgroundColor: "#4c1d95" }}
                  whileTap={{ scale: 0.98 }}
                  variants={mobileItemVariants}
                >
      ...
      >
        <div className="container mx-auto px-4 flex flex-col sm:flex-row justify-between items-center">
          ...
          <div className="text-sm">
            ...
            <Link to="/terms-of-service" className="hover:text-white transition-colors duration-300 ml-2">Terms of Service</Link>
          </div>
        </div>
      </motion.div>
    </footer>
